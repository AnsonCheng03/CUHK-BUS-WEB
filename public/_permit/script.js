    ...
    if (
      ...
      if (form.Type.value == "Transit") {
        ...
        document.querySelector(".card").style.background =
          'url("getcard/images/schbus_d.png")';
        ...
        document.querySelector(".cardname h2").innerText =
          "Meet-Class Bus Permit";
        document.querySelector(".card").style.background =
          'url("getcard/images/schbus_l.png")';
        for (const box of document.querySelectorAll(".routes .transit")) {
          box.style.display = "none";
        }
      }
      document.querySelector(".studatas .Name .value span").innerText = form
        .Name.value
        ? form.Name.value
        : "CHAN Siu-ming （陳小明）";
      document.querySelector(".studatas .SID .value span").innerText = form.SID
        .value
        ? form.SID.value
        : "1155125528";
      document.querySelector(".studatas .Major .value span").innerText = form
        .Major.value
        ? form.Major.value
        : "B.A. in Fine Arts";
      const today = new Date();
      document.querySelector(".studatas .Valid .value span").innerText = form
        .Valid.value
        ? form.Valid.value
        : today.getDate() +
          "/" +
...
function resizecard() {
  ...
  if (size * 356 > document.querySelector(".card").clientHeight)
    size = document.querySelector(".card").clientHeight / 400;
...
document.querySelector(".download").addEventListener("click", () => {
  window.location.href =
    "savecard/?Type=" +
    document.querySelector(".card").getAttribute("Type") +
    "&Name=" +
    document.querySelector(".studatas .Name .value span").innerText +
    "&SID=" +
    document.querySelector(".studatas .SID .value span").innerText +
    "&Major=" +
    document.querySelector(".studatas .Major .value span").innerText +
    "&Valid=" +
    document.querySelector(".studatas .Valid .value span").innerText;
});
